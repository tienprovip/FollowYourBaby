...
import CategoryFilter, { type CategoryFilterValue } from '@/components/milestones/CategoryFilter';
...
export default function MilestonesTab() {
    ...
    try {
      await markAchieved({
        ...
        achieved_at: [
          ...
          String(markDate.getMonth() + 1).padStart(2, '0'),
          String(markDate.getDate()).padStart(2, '0'),
        ].join('-'),
        note: markNote.trim() || null,
      });
      setQuickMark(null);
      setMarkNote('');
      showToast(`Chuc mung! Be da dat moc "${quickMark.catalogItem.title_vi}"`, 'success', 4000);
    } catch {
  ...
  return (
    <>
      <SafeAreaView className="flex-1 bg-brand-peach" edges={['top']}>
        ...
        >
          ...
          <View className="px-5 pb-4 pt-3">
            <Text className="text-brand-navy text-2xl font-bold">Moc phat trien</Text>
            {baby && (
          ...
          <View className="px-5 mb-4">
            <Card padding="md">
              <MilestoneProgress achieved={totalAchieved} total={catalog.length} />
            </Card>
          </View>
          ...
          {overdue.length > 0 && (
            <View className="mx-5 mb-4 bg-amber-50 border border-amber-200 rounded-card px-4 py-3">
              ...
              <Text className="text-amber-700 text-xs leading-5">
                Moi be phat trien khac nhau. Neu ban lo lang, hay trao doi voi bac si nhi khoa.
              </Text>
            </View>
          )}
          ...
          {expectedNow.length > 0 && categoryFilter === 'all' && (
            <View className="px-5 mb-4">
              <View className="flex-row items-center justify-between mb-2">
                <Text className="text-brand-navy font-bold text-base">Du kien thang nay</Text>
                <Text className="text-brand-navy/40 text-xs">{expectedNow.length} moc</Text>
              </View>
          ...
          {recentlyAchieved.length > 0 && categoryFilter === 'all' && (
            <View className="px-5 mb-4">
              <View className="flex-row items-center justify-between mb-2">
                <Text className="text-brand-navy font-bold text-base">Vua dat duoc</Text>
                <TouchableOpacity onPress={() => router.push('/(milestones)/achieved')}>
                  <Text className="text-brand-pink text-xs font-semibold">Xem tat ca</Text>
                </TouchableOpacity>
              </View>
              <View className="gap-y-3">
          ...
          <View className="px-5 mb-3">
            <Text className="text-brand-navy font-bold text-base mb-3">Tat ca cac moc</Text>
            <CategoryFilter
          ...
          <View className="px-5 mt-2 mb-4">
            ...
            <Text className="text-brand-navy/40 text-xs text-center mt-3 leading-5">
              Cac moc tren chi mang tinh tham khao. Moi tre phat trien theo toc do rieng. Hay trao
              doi voi bac si khi co bat ky lo lang nao.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
      ...
      >
        <Text className="text-brand-navy text-lg font-bold mb-1">Danh dau dat</Text>
        {quickMark && (
          <Text className="text-brand-navy/60 text-sm mb-4">{quickMark.catalogItem.title_vi}</Text>
        )}
